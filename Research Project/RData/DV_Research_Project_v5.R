...
plot(dv_pop_mean, type="b", main="Median Average Percentage of NSW \n Population Experiencing Domestic Violence",
     lwd=2, col="darkblue", xlab="Count of Years: 1999 to 2016", ylab="Average",
...
nrow(model_data) #45
model_data <- model_data[,-c(39,51,63,75)]
...
#Rental Data
model_data_rental <- melt(model_data[,c(1:2,61:71)])
colnames(model_data_rental)[3] <- "Rental_Govt"
colnames(model_data_rental)[4] <- "Rental_Govt_ppt"

#Sole Parents
model_data_family_sole_par <- melt(model_data[,c(1:2, 83:93)])
...
model_data_final <- model_data_final[,-c(3,5,6,7,9,10,11,13,14,15,17,18,19,21,22,23)]
...
glm.fit_Rental_Govt <- glm(Domestic_Violence_Count~
                      Rental_Govt_ppt
                      #+ offset(log(Population_Count))
                      , family=poisson(link=log)
                      , data = dataTrain
)
summary(glm.fit_Rental_Govt)

#Now we create our plots. First we set up a sequence of length values which we will use to plot the fitted model. Let's find the range of each variable.
range(dataTrain$Rental_Govt_ppt)

xRental_Govt_ppt <-seq (0, 0.5, 0.01)
yRental_Govt_ppt <- predict(glm.fit_Rental_Govt,
                               list(Rental_Govt_ppt=xRental_Govt_ppt),type="response")

plot(dataTrain$Rental_Govt_ppt, dataTrain$Domestic_Violence_Count, pch = 16, xlab = "Government Rental (x100 for percentage)", ylab = "Domestic Violence Incidents")
lines(xRental_Govt_ppt, yRental_Govt_ppt, col = "red", lwd = 2)
...
#Fit the poisson glm model for all the predictor variables against the outcome variable
...
#STEP THREE: Assess the co-efficients and Confidence Intervals of the model
...
tidy(anova(glm.fit, dispersion=1, test="Chisq"))

#While no exact equivalent to the R2 of linear regression exists, the McFadden R2 index can be used to assess the model fit.
pR2(glm.fit) #Not a bad result at 0.4049991
...
#Income
layout(matrix(c(1,1,2,3),2,2,byrow=T))
plot(glm.fit$resid ~ dataTrain$Income_x800_1249_ppt,
     main="Residuals\nfor Poisson Regression",
...
#Residuals X-squared: 1068.2471 p Value: < 2.2e-16 
...
colnames(output)[9] <- "Prediction"
output$Difference <- (output$Prediction-output$Domestic_Violence_Count)
output <- output[,c(1:3,9:10)]
head(output)
output$Accuracy <- (output$Difference/output$Domestic_Violence_Count)*100
mean(output$Accuracy)
median(output$Accuracy)
